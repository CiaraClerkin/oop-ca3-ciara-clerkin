...
public class MultiCompanyStockSharesTaxCalculation {
    public static void main(String[] args) {
        ...
        while (!command[0].equals("quit")) {
            ...
            if (command[0].equals("buy")) {
                company = command[1];
                quantity = Integer.valueOf(command[2]);
                price = Integer.valueOf(command[3]);

                if (!map.containsKey(company)) {
                    map.put(company, new LinkedList<>());
                }
                map.get(company).add(new Block(quantity, price));
            }
                ...
                for (int i = 0; i < map.get(company).size()+1; i++) {
                    //quantityRecord += shares.peek().getQuantity();
                    //if (quantityRecord <= quantity) {
                    Block currentShare = map.get(company).remove();
